...
void cams_callback(const sensor_msgs::ImageConstPtr &msg0, const sensor_msgs::ImageConstPtr &msg1 , const sensor_msgs::ImageConstPtr& msg2, const sensor_msgs::ImageConstPtr& msg3)
{
...
{
    std::string topic1_name , topic2_name , topic3_name ,topic4_name, output_topic ;
    ros::init(argc,argv,"4cams_synch") ; 
    ros::NodeHandle nh("~") ;
    ...
    auto sync = std::make_shared<message_filters::Synchronizer<sync_pol>>(sync_pol(5), *image_sub0, *image_sub1,*image_sub2,*image_sub3);
    ...
    ROS_INFO("subscribing to cam (quadruple): %s", topic1_name.c_str());
    ROS_INFO("subscribing to cam (quadruple): %s", topic2_name.c_str());
    ROS_INFO("subscribing to cam (quadruple): %s", topic3_name.c_str());
    ROS_INFO("subscribing to cam (quadruple): %s", topic4_name.c_str());
