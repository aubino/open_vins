...
void cams_callback(const sensor_msgs::ImageConstPtr &msg0, const sensor_msgs::ImageConstPtr &msg1 , const sensor_msgs::ImageConstPtr& msg2, const sensor_msgs::ImageConstPtr& msg3, const sensor_msgs::ImageConstPtr& msg4)
{
...
{
    std::string topic1_name , topic2_name , topic3_name ,topic4_name, topic5_name, output_topic ;
    ros::init(argc,argv,"5cams_synch") ; 
    ros::NodeHandle nh("~") ;
    ...
    {
        ROS_INFO("Could not get the name of the cam2topic. Defaulting to %s","/cam3") ; 
        topic4_name = "/cam3" ; 
    }

     if(! nh.getParam("cam4topic",topic5_name)) 
    {
        ROS_INFO("Could not get the name of the cam2topic. Defaulting to %s","/cam4") ; 
        topic5_name = "/cam4" ; 
    }
    ...
    auto sync = std::make_shared<message_filters::Synchronizer<sync_pol>>(sync_pol(5), *image_sub0, *image_sub1,*image_sub2,*image_sub3,*image_sub4);
    ...
    ROS_INFO("subscribing to cam (quintuple): %s", topic1_name.c_str());
    ROS_INFO("subscribing to cam (quintuple): %s", topic2_name.c_str());
    ROS_INFO("subscribing to cam (quintuple): %s", topic3_name.c_str());
    ROS_INFO("subscribing to cam (quintuple): %s", topic4_name.c_str());
    ROS_INFO("subscribing to cam (quintuple): %s", topic5_name.c_str());
