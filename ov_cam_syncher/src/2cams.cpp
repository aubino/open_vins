...
{
    std::string topic1_name , topic2_name , output_topic ;
    ros::init(argc,argv,"2cams_synch") ; 
    ros::NodeHandle nh("~") ;
    ...
    ROS_INFO("subscribing to cam (stereo): %s", topic1_name.c_str());
    ROS_INFO("subscribing to cam (stereo): %s", topic2_name.c_str());
