...
struct CamerasTrackerFactory {
  ...
  const std::map <std::string, dai::MonoCameraProperties::SensorResolution> sensor_resolutiion_map {{"400p" : dai::MonoCameraProperties::SensorResolution::THE_400_P},
  ...
  std::vector<TrackerOptions> tracker_list ; 

  /**
   * @brief Function to convert tracker config files into node 
   */
  std::vector<dai::node::MonoCamera> createCameras( std::shared_ptr<dai::Pipeline> pipeline , 
                                                    const std::shared_ptr<ov_dai::YamlParser> &parser)
  {
    if (parser != nullptr)
    {
   ...
   * @param parser If not null, this parser will be used to load our parameters
   */
  void print_and_load_state(const std::shared_ptr<ov_dai::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
